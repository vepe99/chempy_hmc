"""Corner plot of the global Lambda = [alpha_IMF, log10 N_Ia] posterior samples,
with a Gaussianized approximation (mean + covariance) overlaid as 1/2-sigma error
ellipses and Gaussian marginals.

Usage: python corner_gauss.py --n_stars 200 [--tag 200_3k]
"""
import argparse
import os
import numpy as np
...
def nsigma_ellipse(ax, mu, cov, n, **kw):
    """Draw the n-sigma error ellipse (semi-axes = n*sqrt(eigenvalues))."""
    vals, vecs = np.linalg.eigh(cov)
    order = vals.argsort()[::-1]
    vals, vecs = vals[order], vecs[:, order]
    angle = np.degrees(np.arctan2(vecs[1, 0], vecs[0, 0]))
    width, height = 2 * n * np.sqrt(vals)  # full axis lengths
...
def main():
    ...
    print(f"N={args.n_stars}  (tag={args.tag or 'main'})   n_samples={len(lam)}")
    ...
    # Diagonal marginals: histogram + Gaussian approx
    for i in range(2):
        ax = axes[i, i]
    ...
    # Lower-left: joint scatter/2D-hist + 1/2-sigma ellipses
    ax = axes[1, 0]
